    ...
    const onSubmitHandler = (e) => {
        ...
        setTimeout(() => {
            axios.get(`https://api.github.com/search/users?q=${input}`)
                .then(res => { setUserSearch(res.data.items); setTotalItemCount(res.data) })
            setLoading(false);
            console.log(userSearch)
        }, 1200)
    }
    // change pagination accoridng to search user
    const handlePageClick = (data) => {
        let currentPage = data.selected + 1;
        axios.get(`https://api.github.com/search/users?q=${input}&page=${currentPage}`)
...
// these user.avatar-url, user.name we will get that from api okay...
